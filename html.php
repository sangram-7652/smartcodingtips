...
// ✅ Base URL function (works on localhost and live)
function base_url($path = '') {
    $host = $_SERVER['HTTP_HOST'];
    $isLocal = $host === 'localhost' || str_contains($host, '127.0.0.1');
    $base = $isLocal ? '/smartcodingtips' : ''; // update if your local folder is different
...
$keywords = $keywords ?? "html, web development, coding tips";

// ✅ Canonical (SEO URL)
$slug = basename($page);
$canonical = "https://smartcodingtips.com/html/$slug";
...
// ✅ TOC Link function (uses base_url)
function tocLink($label, $link, $currentPage)
{
    $slug = basename($link); // html/what-is-html → what-is-html
    $isActive = ($link === $currentPage);
    $activeClass = $isActive ? 'bg-yellow-300 dark:bg-yellow-600 text-black font-semibold rounded px-2' : '';
    echo "<li><a href='" . base_url("html/$slug") . "' class='block px-2 py-1 rounded $activeClass'>$label</a></li>";
}
?>
