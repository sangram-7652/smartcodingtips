...
// ✅ Helper for local or live base URL
function base_url($path = '')
{
    $host = $_SERVER['HTTP_HOST'];
    $isLocal = $host === 'localhost' || str_contains($host, '127.0.0.1');
    $base = $isLocal ? '/smartcodingtips' : ''; // adjust if folder is different
...
// ✅ SEO meta (optional overrides can be placed in page content)
$title = $title ?? "JavaScript Tutorial - SmartCodingTips";
$description = $description ?? "Learn JavaScript from beginner to advanced with interactive examples.";
$keywords = $keywords ?? "javascript, learn js, web development";
$slug = basename($page);
$canonical = "https://smartcodingtips.com/js/$slug";

// ✅ Current TOC page for highlighting
...
{
    $slug = basename($link); // js/what-is-js → what-is-js
    $activeClass = ($link === $currentPage) ? 'bg-yellow-300 dark:bg-yellow-600 text-black font-semibold rounded px-2' : '';
    echo "<li><a href='" . base_url("js/$slug") . "' class='block px-2 py-1 rounded $activeClass'>$label</a></li>";
}
?>
